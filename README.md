...
- 添加 "@" 特殊搜索前缀（只能用于任务搜索，不能用于文件列表）
- 切换页面时清空搜索关键词

特殊搜索示例：  
@8 搜索包含 8 个及以上文件，正在下载的任务。  
@8c 搜索包含 8 个及以上文件的所有任务。包括已经下载完成，但还在上传的任务。  
@8a 搜索包含 8 个及以上文件并且已经选中所有文件的任务。  
@8s 搜索 seeder 大于等于 8 的任务。  
@8n 搜索连接数 大于等于 8 的任务。  
@80% 搜索完成度小于 80% 的任务。

_注： c 可以和其他后缀组合使用，[a, s, n, %] 只可以选一个，前面添加`!`表示取反_
